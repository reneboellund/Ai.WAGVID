...
from ai_wagvid.full_apparatus_promotion import evaluate_full_apparatus_promotion
...
def base_inputs():
    ...
    benchmark = ApparatusBenchmarkReport(
        ...
        slices=(BenchmarkSliceResult(
            ...
            metrics=(BenchmarkMetric("top1", 950, 900, True),), status=SliceStatus.PASS, sample_count=20,
        ),),
        required_slice_ids=("required",), executed_at=T0,
    )
    rulepack = RulepackReadiness(
        rulepack_id=binding.rulepack_id, ready=True, blockers=(),
        manifest_sha256="c"*64, source_ids=("source",),
    )
    return model, binding, facts, ledger, benchmark, rulepack


def test_non_ready_model_profile_blocks_otherwise_passing_stack():
    model, binding, facts, ledger, benchmark, rulepack = base_inputs()
    profile = ProfileReadiness(
        profile_id="competition-profile",
        ready=False,
        blockers=("rtmpose:component-checkpoint-digest-missing",),
        components=(),
    )
    decision = evaluate_full_apparatus_promotion(
        model=model, model_profile_readiness=profile, binding=binding, accepted_facts=facts,
        benchmark=benchmark, dscore_ledger=ledger, rulepack_readiness=rulepack,
    )
    assert decision.status is PromotionStatus.BLOCKED
    assert "model-profile-not-benchmark-ready" in decision.blockers
    assert "model:rtmpose:component-checkpoint-digest-missing" in decision.blockers


def test_all_readiness_layers_allow_integrated_post_routine_status():
    model, binding, facts, ledger, benchmark, rulepack = base_inputs()
    profile = ProfileReadiness(
        profile_id="competition-profile", ready=True, blockers=(), components=(),
    )
    decision = evaluate_full_apparatus_promotion(
        model=model, model_profile_readiness=profile, binding=binding, accepted_facts=facts,
        benchmark=benchmark, dscore_ledger=ledger, rulepack_readiness=rulepack,
