...
CRUD/archive operations are kept separate from capture/review views. Every mutation is permission
checked and append-only audited; historical records are deactivated/archived rather than deleted.
"""
...
def gymnasts(request):
    ...
    return render(
        ...
        {"organization": organization, "gymnasts": queryset, "can_manage": _can_manage(request, organization)},
    )
...
def gymnast_edit(request, gymnast_id):
    ...
    if request.method == "POST" and form.is_valid():
        ...
        updated = form.save()
        _audit(organization, request, "gymnast.updated", "gymnast", updated.id, metadata={"changed_fields": changed})
        messages.success(request, f"{updated.display_name} er opdateret og audit-logget.")
        return redirect("gymnasts")
    return render(
        request,
        "wagvid/form.html",
        {"title": f"Redigér {gymnast.display_name}", "form": form, "organization": organization},
    )
...
def gymnast_archive(request, gymnast_id):
    ...
    if gymnast.archived_at is None:
        ...
        _audit(organization, request, "gymnast.archived", "gymnast", gymnast.id, reason=request.POST.get("reason", ""))
        messages.success(request, f"{gymnast.display_name} er arkiveret. Historiske data er bevaret.")
    return redirect("gymnasts")
...
def levels(request):
    ...
    return render(
        request,
        "wagvid/levels.html",
        {"organization": organization, "levels": organization.levels.all(), "can_manage": _can_manage(request, organization)},
    )
...
def level_create(request):
    ...
    return render(
        ...
        {"title": "Opret niveau", "form": form, "organization": organization, "back_url_name": "levels"},
    )
...
def level_edit(request, level_id):
    ...
    if request.method == "POST" and form.is_valid():
        changed = sorted(form.changed_data)
        updated = form.save()
        _audit(organization, request, "level.updated", "level", updated.id, metadata={"changed_fields": changed})
        messages.success(request, f"Niveauet {updated.name} er opdateret.")
        return redirect("levels")
    return render(
        request,
        "wagvid/master_form.html",
        {"title": f"Redigér niveau: {level.name}", "form": form, "organization": organization, "back_url_name": "levels"},
    )
...
def level_archive(request, level_id):
    ...
    if level.active:
        ...
        messages.success(request, f"Niveauet {level.name} er deaktiveret; historiske profiler er bevaret.")
    return redirect("levels")
