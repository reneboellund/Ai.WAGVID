"""Orchestration between Wasabi data-plane operations and the durable object ledger."""
...
from .wasabi import BucketRole
from .wasabi_provider import create_boto3_client
...
def upload_registered_object(
    *,
    ...
    if bucket.state != StorageBucket.State.READY:
        raise ValueError("selected Wasabi bucket is not ready")
    ...
    organization.audit_events.create(
        ...
        metadata={"bucket": bucket.bucket_name, "size_bytes": stored.size, "role": role.value},
    )
    return record
